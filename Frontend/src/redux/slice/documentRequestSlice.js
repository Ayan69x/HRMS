   ...
   // Fetch token inside thunk
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL || 'http://localhost:8080'}/api/create`,
        requestData, {
          headers: {
  ...
  async (_, thunkAPI) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL || 'http://localhost:8080'}/api/employee/requests`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        }
  ...
  async ({ id, updateData }, thunkAPI) => {
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_API_URL || 'http://localhost:8080'}/api/update/${id}`,
        updateData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  async (id, thunkAPI) => {
    try {
      const response = await axios.delete(
        `${process.env.REACT_APP_API_URL || 'http://localhost:8080'}/api/delete/${id}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        }
...
export const submitDocument = createAsyncThunk(
  ...
  async ({ documentRequestId, file }, thunkAPI) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_API_URL || 'http://localhost:8080'}/api/submit/${documentRequestId}`,
        formData,
        {
          headers: {
...
export const getAllDocumentRequests = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const response = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:8080'}/api/hr/requests`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
export const getSubmissionsForReview = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL || 'http://localhost:8080'}/api/hr/submissions`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        }
...
export const reviewSubmission = createAsyncThunk(
  ...
  async ({ id, reviewData }, thunkAPI) => {
    try {
      const response = await axios.put(
        `${process.env.REACT_APP_API_URL || 'http://localhost:8080'}/api/hr/review/${id}`,
        reviewData, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
