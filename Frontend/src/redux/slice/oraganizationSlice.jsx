...
export const setupOrganization = createAsyncThunk(
  ...
  async (formData, { rejectWithValue }) => {
    try {
      ...
      // API call
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:8080'}/auth/addOrganization`, {
        method: 'POST',
        body: data,
      });
...
export const fetchOrganizationById = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:8080'}/auth/addOrganization/view/${id}`);
